        ...
        let expires_serializable: HashMap<String, u64> = db.expires
            ...
            .filter_map(|(key, instant)| {
                if *instant > now_instant {
                    // Calculate how many seconds from now until expiry
        ...
        if let Ok(current_secs) = now_system.duration_since(UNIX_EPOCH) {
            for (key, expire_timestamp) in persisted_data.expires {
                if expire_timestamp > current_secs.as_secs() {
                    ...
                    let expiry_instant = now_instant + Duration::from_secs(seconds_until_expiry);
                    expires.insert(key, expiry_instant);
                }
                // Skip expired keys
            }
        }
